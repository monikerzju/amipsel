...
class PCGen(va_width: Int = 32, start_va: String = "h80000000", increment: Int = 4) extends Module with Config {
  ...
  val inner_npc = Mux(io.please_wait, pc, Mux(bpu.io.resp.taken_vec(0), bpu.io.resp.target_first, pc + increment.U))
  val npc = Mux(io.redirect, io.redirect_pc, inner_npc)
...
class Frontend(diffTestV: Boolean) extends Module with Config with MemAccessType with FrontToBack {
  ...
  // ID
  val decode_pc_low_reg = Reg(UInt(len.W))
  ...
  cache_stall    := !io.icache.resp.valid
  ...
  // ce for common expression
  val stall_f_repc_ce = Mux(stall_f, repc, pc_gen.io.pc_o)
  val stall_f_reptar_ce = Mux(stall_f, reptar, pc_gen.io.predict_target_o)
  val stall_f_repred_ce = Mux(stall_f, repred, pc_gen.io.predict_taken_o)
  repc := stall_f_repc_ce
  reptar := stall_f_reptar_ce
  repred := stall_f_repred_ce
  if (frontendIssueN != 1) {
    renarrow := Mux(stall_f, renarrow, pc_gen.io.narrow_o)
  }

  val may_illegal_req_addr = stall_f_repc_ce
  ...
  decode_valid_reg := Mux(kill_d, false.B, Mux(!stall_d && !cache_stall, io.icache.req.valid, decode_valid_reg))
  when (!stall_d && !cache_stall) {
    decode_pc_low_reg := may_illegal_req_addr
    decode_pc_predict_target := stall_f_reptar_ce
    decode_pc_predict_taken  := stall_f_repred_ce
  }

  frontend_fire := !cache_stall && decode_valid_reg
  io.fb.fmbs.instn := Mux(stall_d || !frontend_fire, 0.U, fire_number_respn) // here resp number can be inferred from the address, if cross line 0, else 1. The actual number is respn + 1.
  ...
  predict_taken_but_not_br := decode_pc_predict_taken && (io.icache.resp.bits.rdata(0)(31, 26) === "b000001".U || io.icache.resp.bits.rdata(0)(31, 28) === "b0001".U)
