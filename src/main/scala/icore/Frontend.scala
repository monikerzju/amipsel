...
class Frontend extends Module with Config with MemAccessType with FrontToBack {
  ...
  val icache_stall_req_a = (if (metaZeroLatency) RegNext(!io.icache.resp.valid) else !io.icache.resp.valid) && last_req_valid
  val icache_fetch_half_a = !io.icache.resp.bits.respn    // Combo logic
  val wtg = last_wait
  ...
  val wait_icache = icache_stall_req_a
  pc_gen.io.please_wait := wait_icache || io.fb.fmbs.please_wait
  pc_gen.io.redirect := io.fb.bmfs.redirect_kill || (icache_fetch_half_a && !wait_icache)  // TODO when BPU is added
  ...
  io.icache.req.bits.addr := Mux(icache_stall_req_a, repc, pc_gen.io.pc_o)
  ...
  decode_instn := Mux(
    io.fb.bmfs.redirect_kill, 0.U, 
    Mux(io.fb.fmbs.please_wait, decode_instn, Cat(0.U, io.icache.resp.bits.respn) + 1.U)
  )
  // some IO to the fifo backend
  io.fb.fmbs.instn := Mux(io.fb.fmbs.please_wait, 0.U, Mux(!wait_icache && !delayed_redirect_kill, decode_instn, 0.U))
  for (i <- 0 until frontendIssueN) {
    decs(i).inst := Mux(icache_stall_req_a, decode_reg_line(i), io.icache.resp.bits.rdata(i))
