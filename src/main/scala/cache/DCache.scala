/***********************************************************
*********************dcache prototype***********************
1-way direct-mapped data cache
features:   
...
import icore._
import conf._
class AXI4IO_DataReq extends Bundle with Config{
  val addr = Output(UInt(len.W))
  val wdata = Output(UInt(len.W))
  val wen = Output(Bool())
//   val type = Output(UInt(SZ_MEM_TYPE.W))
}
class AXI4IO_Data extends Bundle {
    val req=Flipped(Decoupled(new AXI4IO_DataReq))
    val resp=Decoupled(new MemResp)
}

class DCache1Way(nline:Int,size:Int) extends Module with Cache_Parameters{
